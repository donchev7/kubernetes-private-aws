...
BASE_PROXY_SCHEMA="http"
BASE_PROXY_HOST="cias.geoaws.com"
BASE_PROXY_PORT="8080"
BASE_PROXY="$BASE_PROXY_SCHEMA://$BASE_PROXY_HOST:$BASE_PROXY_PORT/"
...
# Set apt-get proxy
echo "Acquire::http::Proxy \"http://cias.geoaws.com:8080\";" >> /etc/apt/apt.conf
echo "Acquire::https::Proxy \"http://cias.geoaws.com:8080\";" >> /etc/apt/apt.conf
...
apt-get update -y
apt-get -y install iptables-persistent libevent-dev awscli jq

aws s3 cp s3://${s3_id}/bin/redsocks . --region eu-central-1
chmod +x redsocks
